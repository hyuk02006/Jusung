...
DWORD WINAPI ReadThread(LPVOID p)
{
	...
	for (;;)
	{
		...
		g_ctrlList->AddString((CString)szInput);
...
DWORD WINAPI ListenThread(LPVOID p)
{
	...
	for (;;)//고객을 여러명 상대하는 for문
	{
		...
		hPipe = CreateNamedPipe(_T("\\\\.\\pipe\\myexam"),
			PIPE_ACCESS_DUPLEX,
			PIPE_TYPE_BYTE,
			3, //인스턴스 최대수
			4096, 0, //out /in buff
		...
		if (bConnect == TRUE)
		{
			CloseHandle(CreateThread(NULL, 0, ReadThread, (LPVOID)hPipe, 0, 0));
		}
		else
		{
			CloseHandle(hPipe);
...
{
	g_ctrlList = &m_ctrlList;
	m_ctrlList.SetTopIndex(m_ctrlList.GetCount() - 1);
