...
BEGIN_MESSAGE_MAP(CMemShare2Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_EN_CHANGE(IDC_EDIT1, &CMemShare2Dlg::OnEnChangeEdit)
	ON_WM_CREATE()
	ON_WM_DESTROY()
...
int CMemShare2Dlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//1. 파일 매핑 오브젝트를 생성한다.
	hFMap = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 1024, _T("MyMemShareMapping"));
...
void CMemShare2Dlg::OnEnChangeEdit()
{
	...
	if (pWnd)pWnd->PostMessage(WM_USER + 1, 0, 0);
}
