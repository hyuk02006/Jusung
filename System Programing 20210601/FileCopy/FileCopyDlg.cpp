...
BEGIN_MESSAGE_MAP(CFileCopyDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_COPY, &CFileCopyDlg::OnBnClickedButtonCopy)
END_MESSAGE_MAP()
...
void CFileCopyDlg::OnBnClickedButtonCopy()
{
	//1. 소스 파일의 핸들을 생성한다.
	HANDLE hSrc;
	...
	//3. 타켓 파일의 핸들을 생성한다.
	HANDLE hDest;
	hDest = CreateFile(_T("C:\\temp\\Core.xml"),  GENERIC_WRITE,0,NULL,CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,NULL);
	//4. for문을 돌면서 복사한다.
	BYTE byBuff[8192];
