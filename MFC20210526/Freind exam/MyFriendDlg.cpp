...
void CMyFriendDlg::OnBnClickedButtonAdd()
{
	...
	m_ctrlFriend.InsertItem(nCount, m_strName,m_bGender);
	m_ctrlFriend.SetItemText(nCount,1,strTemp);

	if(m_bGender == 0)
		m_ctrlFriend.SetItemText(nCount, 2, _T("남자"));
	else
		m_ctrlFriend.SetItemText(nCount, 2, _T("여자"));

	//if (((CButton*)GetDlgItem(IDC_RADIO_MAN))->GetCheck()==TRUE)
	//	((CListCtrl*)GetDlgItem(IDC_LIST_FRIEND))->SetItemText(nCount, 2, _T("남자"));
	//else
	//	((CListCtrl*)GetDlgItem(IDC_LIST_FRIEND))->SetItemText(nCount, 2, _T("여자"));
	nCount++;
