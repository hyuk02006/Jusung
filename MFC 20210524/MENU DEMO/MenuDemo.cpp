...
{
	if (m_bPlay)//지금 재생 중인가?
	{
		pCmdUI->Enable(0);//Disable
	}
	else
	{
		pCmdUI->Enable(1);//Enable
	}
}
...
void CMenuDemoApp::OnMusicPause()
{
	...
	if (m_bPause)	//지금 일시 정지 중인가
	{
		AfxMessageBox(_T("다시 시작합니다.")); //다시 재생
	}
	else
	{
		AfxMessageBox(_T("일시 정지 합니다.")); //일지 정지
	}
...
void CMenuDemoApp::OnUpdateMusicPause(CCmdUI* pCmdUI)
{
	...
	if (m_bPause) //지금 일시정지 중인가?
	{
		pCmdUI->SetText(_T("다시 시작"));
		pCmdUI->SetCheck(1);
	}
	else
	{
