...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST, m_ctrlBox);
}
...
BEGIN_MESSAGE_MAP(CEnumProcessesDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_START, &CEnumProcessesDlg::OnBnClickedButtonStart)
END_MESSAGE_MAP()
...
void CEnumProcessesDlg::OnBnClickedButtonStart()
{
	DWORD pidArray[512] = { 0, };
	DWORD cdNeeded;

	EnumProcesses(pidArray, sizeof(pidArray), &cdNeeded); //지금 떠있는 프로세스의 아이디값을 모음 

	int nProcesses = cdNeeded / sizeof(DWORD);	//프로세스의 개수
